...
const Terminal = () => {
    ...
    const commands = {
        ...
        ls: (args) => {
            ...
            return { result: [...(files[dir] || [`ls: ${dir}: No such directory`]), ''] };
        },
    ...
    return (_jsxs("div", { className: `terminal-content ${isClearing ? 'clearing' : ''}`, children: [_jsx("div", { className: "terminal-output", ref: outputRef, children: output.map((line, i) => (_jsx("div", { className: `terminal-line ${line.startsWith('ls:') || line.startsWith('cd:') || line.startsWith('cat:') ? 'error' : ''} ${line.endsWith('/') ? 'dir' : ''}`, children: line }, i))) }), _jsxs("form", { onSubmit: handleSubmit, className: "terminal-input", children: [_jsx("span", { className: "prompt", children: "$" }), _jsx("input", { type: "text", ref: inputRef, value: input, onChange: (e) => setInput(e.target.value), onKeyDown: handleKeyDown, className: "terminal-input-field", autoFocus: true })] })] }));
};
